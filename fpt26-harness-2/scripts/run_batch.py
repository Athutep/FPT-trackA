...
# Windows console default (gbk) cannot encode unicode output; force UTF-8.
...
def run_one(
    ...
    try:
        ...
        (work_root / tid / f"final_{task.kernel_name}").write_text(final, encoding="utf-8")
        return {
            "task_id": tid,
            "type": task.type,
...
def write_summary(
    ...
    # Aggregate token consumption (provider usage, cache-aware).
    total_tok = sum(r.get("tokens", 0) for r in results.values())
    total_prompt = sum(r.get("prompt_tokens", 0) for r in results.values())
    total_comp = sum(r.get("completion_tokens", 0) for r in results.values())
    total_cached = sum(r.get("cached_tokens", 0) for r in results.values())
    total_api = sum(r.get("api_calls", 0) for r in results.values())
    total_fails = sum(r.get("api_failures", 0) for r in results.values())
    ...
    lines = [
        ...
        f"- LLM tokens: {total_tok} total "
        f"({total_prompt} prompt + {total_comp} completion, {total_cached} cached)",
        f"- LLM api: {total_api} calls, {total_fails} failed attempts",
        "",
