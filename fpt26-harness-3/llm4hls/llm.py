...
class ScriptedClient:
    ...
    def __init__(self, responses: list[str]) -> None:
        ...
        self.api_calls = 0
        self.api_failures = 0
        # token accounting (always zero: no real API)
        self.total_tokens = 0
    ...
    def token_summary(self) -> str:
        return "tokens: 0 (scripted backend)"
...
class OpenRouterClient:
    ...
    def __init__(
        ...
        # token accounting (from provider `usage`; DeepSeek reports cached hits)
        self.total_tokens = 0
        self.prompt_tokens = 0
        self.completion_tokens = 0
        self.cached_tokens = 0
    ...
    def token_summary(self) -> str:
        """One-line token consumption report (cache-aware)."""
        if self.api_calls == 0:
            return "tokens: 0 (no API calls)"
        return (
    ...
    def complete(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # DeepSeek / OpenRouter report prefix-cache hits separately.
