...
# Max single-resource utilization (%) allowed for a promoted candidate.
MAX_RESOURCE_UTIL_PCT = float(os.environ.get("LLM4HLS_MAX_UTIL_PCT", "90"))

# --- Token budget (submission requires token accounting & limits) ----------
# Hard cap on total LLM tokens per agent run; None disables the limit.
# 0 disables too (unlimited). Set a real number to enforce a ceiling.
TOKEN_BUDGET = (
    int(os.environ["LLM4HLS_TOKEN_BUDGET"])
    if os.environ.get("LLM4HLS_TOKEN_BUDGET")
    else None
)
