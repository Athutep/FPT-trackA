...
# Two adjacent single-statement for loops, each writing to a *different*
# hls::stream. In a DATAFLOW region this is the classic burst-write deadlock:
# one stream is filled entirely before the other starts, overflowing the
# bounded (depth-2) RTL FIFOs. The rule merges them into one loop.
_TWO_BURST_LOOPS_RE = re.compile(
    r"for\s*\((?P<i1>[^;]+);(?P<c1>[^;]+);(?P<inc1>[^)]+)\)\s*"
    r"(?P<body1>[^;{}]*?\.write\s*\([^)]*\))\s*;\s*"
    r"for\s*\((?P<i2>[^;]+);(?P<c2>[^;]+);(?P<inc2>[^)]+)\)\s*"
    r"(?P<body2>[^;{}]*?\.write\s*\([^)]*\))\s*;",
    re.DOTALL,
)
...
# Scarcer / more structurally costly resources weigh more in the area term.
# (Mirrors the reference v2.3 qhw.py: DSP/BRAM/URAM >> LUT/FF on U55C.)
_QHW_AREA_WEIGHTS = {
    "LUT": 1.0,
    "FF": 0.5,
    "DSP": 4.0,
...
# Hard gate: any single-resource utilization above this rejects a candidate.
MAX_RESOURCE_UTIL_PCT = 90.0
...
class ReferenceAgent:
    def __init__(
        ...
        # Hard ceiling on total LLM tokens for this run (None/0 = unlimited).
        self.token_budget = token_budget if token_budget else config.TOKEN_BUDGET
    ...
    def _token_over_budget(self) -> bool:
        """True if the LLM has already consumed more than the token budget."""
        if not self.token_budget:
            return False
        used = getattr(self.llm, "total_tokens", 0) or 0
    ...
    def _meets_fmax(self, report: SynthReport | None) -> bool:
        ...
        # clock_period_ns can be 0.0 for combinational designs (no registered
        # path); treat that as unknown rather than dividing by zero.
        if (
            report is None
            or report.clock_period_ns is None
            or report.clock_period_ns <= 0
        ):
    ...
    def _compute_qhw(
        ...
        # --- area: penalize resource growth vs baseline, weighted by scarcity.
        # Uses absolute resource *ratios* (used/base), not utilization-point
        # deltas — utilization% on the huge U55C understates real cost growth.
        area = 1.0
        if base_report is not None:
            penalty = 0.0
            for key, wgt in _QHW_AREA_WEIGHTS.items():
                used = cand_report.resources.get(key, 0) or 0
                base = base_report.resources.get(key, 0) or 0
                if used <= 0 and base <= 0:
                    continue
                ratio = (used + 1.0) / (base + 1.0)  # +1 guards tiny baselines
                penalty += wgt * max(0.0, ratio - 1.0)
            area = 1.0 / (1.0 + penalty)

        # --- timing: half absolute (vs target clock), half relative (vs baseline) ---
        timing = 0.5  # neutral fallback when clocks are missing
        target_ns = self.task.clock_ns
        if cand_report.clock_period_ns and cand_report.clock_period_ns > 0:
            t_abs = min(target_ns / cand_report.clock_period_ns, 1.0)
            if (
                base_report is not None
                and base_report.clock_period_ns
                and base_report.clock_period_ns > 0
            ):
                t_rel = min(base_report.clock_period_ns / cand_report.clock_period_ns, 1.0)
            else:
                t_rel = t_abs
            timing = 0.5 * t_abs + 0.5 * t_rel

        return wp * perf + wa * area + wt * timing

    def _check_resource_gates(self, report: SynthReport | None) -> tuple[bool, str]:
        """Hard gate: reject candidates whose per-resource utilization is too high.

        Returns (ok, reason). Mirrors the reference v2.3 check_gates() —
        a design that pins 95% of a scarce resource is not a viable build.
        """
        if report is None:
            return False, "no synthesis report"
        for k, u in report.utilization.items():
            if u is not None and u > MAX_RESOURCE_UTIL_PCT:
                return False, f"{k} utilization {u:.0f}% > {MAX_RESOURCE_UTIL_PCT:.0f}%"
        return True, ""
    ...
    # -- rule planner (deterministic fixes, no LLM) ------------------------

    def _fix_missing_includes(self, code: str) -> str | None:
        """Add missing HLS headers when types are used without their include."""
        additions: list[str] = []
        if re.search(r"\bhls::stream\b", code) and "#include <hls_stream.h>" not in code:
            additions.append("#include <hls_stream.h>")
        if re.search(r"\bap_(u?int)\b", code) and "#include <ap_int.h>" not in code:
            additions.append("#include <ap_int.h>")
        if re.search(r"\bap_(u?fixed)\b", code) and "#include <ap_fixed.h>" not in code:
            additions.append("#include <ap_fixed.h>")
        if not additions:
            return None
    ...
    def _fix_stream_order(self, code: str) -> str | None:
        """Merge two burst stream-write loops that deadlock bounded RTL FIFOs.

        Matches a DATAFLOW stage that writes one hls::stream in a full loop and
        then another in a second loop. Rewriting both writes into a single loop
        keeps producers rate-balanced, which is what the RTL FIFOs require.
        """
        m = _TWO_BURST_LOOPS_RE.search(code)
        if not m:
            return None
        body1 = m.group("body1")
        body2 = m.group("body2")
        dst1 = re.match(r"\s*(\w+)\.write", body1)
        dst2 = re.match(r"\s*(\w+)\.write", body2)
        if not dst1 or not dst2 or dst1.group(1) == dst2.group(1):
            return None
        merged = (
            f"for ({m.group('i1')}; {m.group('c1')}; {m.group('inc1')}) {{\n"
            f"        {body1};\n"
            f"        {body2};\n"
            f"    }}"
        )
        return code[: m.start()] + merged + code[m.end():]

    def _rule_planner(self, code: str, r: ToolResult) -> str | None:
        """Try deterministic rule-based fixes before calling the LLM.

        Returns a fixed kernel, or None if no rule matches (the caller then
        falls back to the LLM). Only safe, high-confidence transformations are
        implemented here; anything uncertain goes to the model.
        """
        err_type = _classify_error(r)

        # Rule 1: compile errors from missing HLS headers.
        if err_type == "COMPILE_ERROR":
            fixed = self._fix_missing_includes(code)
            if fixed and fixed != code:
                self._log("rule: added missing HLS header include(s)")
                return fixed

        # Rule 2: DATAFLOW burst-write deadlock (structural tasks).
        if err_type in ("COSIM_FAIL", "DEADLOCK") and self.task.requires_cosim:
            fixed = self._fix_stream_order(code)
            if fixed and fixed != code:
                self._log("rule: merged burst stream-write loops (deadlock fix)")
                return fixed
    ...
    def _reach_correctness(self, code: str, max_rounds: int | None = None) -> tuple[bool, str]:
        ...
        for attempt in range(1, cap + 1):
            if not self._afford("csim"):
                ...
                return False, code
            if self._token_over_budget():
                self._log("LLM token budget exhausted; stopping repair")
                return False, code
            ...
            # Try deterministic rule-based fix first (no LLM API cost).
            rule_fix = self._rule_planner(code, r)
            if rule_fix is not None:
                code = rule_fix
                ok, r = self._verify_correctness(code)
                self._log(f"repair attempt {attempt} (rule): {r.brief()}")
                ...
                if ok:
                    ...
                    self._log(f"correctness reached ({gate}) via rule")
                    return True, code
                continue
    ...
    def _optimize(self, best: str, best_latency: int | None,
        ...
        while (
            ...
            and not self._token_over_budget()
        ):
            rounds += 1
            ...
            if not self._meets_fmax(sr.report):
                ...
                stalls += 1
            elif not self._check_resource_gates(sr.report)[0]:
                _, reason = self._check_resource_gates(sr.report)
                self._log(
                    f"opt round {rounds}: latency {best_latency}->{lat} but "
                    f"{reason}; rejected (resource gate)"
                )
                stalls += 1
    ...
    # -- evidence (traceability for the submission report) ----------------

    def _write_evidence(self, final_code: str) -> None:
        """Write a human-readable EVIDENCE.md: budget, tool transcript,
        token usage and final-kernel SHA-256. Pure side-effect, never throws."""
        try:
            import hashlib
            from datetime import datetime
            sha = hashlib.sha256(final_code.encode("utf-8")).hexdigest()[:16]
            lines = [
                f"# EVIDENCE — {self.task.id}",
                f"- date: {datetime.now():%Y-%m-%d %H:%M}",
                f"- task: {self.task.id} ({self.task.type}, difficulty {self.task.difficulty})",
                f"- target: {self.task.part} @ {self.task.clock_ns} ns",
                f"- budget: {self.server.budget.spent}/{self.server.budget.total} credits",
                f"- final kernel SHA-256: {sha}",
                f"- LLM tokens: {getattr(self.llm, 'total_tokens', 0)} "
                f"(cached {getattr(self.llm, 'cached_tokens', 0)})",
                f"- LLM calls: {getattr(self.llm, 'api_calls', 0)}, "
                f"failures: {getattr(self.llm, 'api_failures', 0)}",
                "",
                "## Tool transcript",
                "",
                "| # | tool | phase | credits |",
            ...
            for e in self.server.transcript:
                lines.append(f"| {e.n} | {e.kind} | {e.phase} | {e.spent} |")
            ev = self.server.run_root / "EVIDENCE.md"
